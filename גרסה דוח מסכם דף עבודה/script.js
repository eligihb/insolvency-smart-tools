...
function updateTransparencyIndex() {
  ...
  if (transparencyGauge && transparencyText && transparencyValue) {
    ...
    if (!hasData) {
      // אין נתונים
      transparencyValue.textContent = 'אין נתונים';
      transparencyValue.style.display = 'block';
      transparencyText.textContent = '';
      transparencyGauge.style.width = '0%';
      transparencyGauge.style.backgroundColor = '#e2e8f0';
      const statusTextEl = document.getElementById('transparencyStatusText');
      if (statusTextEl) statusTextEl.textContent = '';
      return;
    }
    ...
    // עדכון הטקסט - הצגת אחוזים בתוך הסרגל
    const percentText = transparency + '%';
    transparencyText.textContent = percentText;
    
    // עדכון ערך מדד שקיפות
    transparencyValue.textContent = percentText;
    transparencyValue.style.display = 'block';
...
function updateArrearsGauge(compliancePercent, arrearsAmount, hasCredit = false) {
  ...
  if (arrearsGauge && arrearsText && arrearsValue) {
    ...
    const arrearsStatusText = document.getElementById('arrearsStatusText');
    
    if (arrearsValue) {
      if (monthsSinceOrder > 0 && monthlyPayment > 0 && compliancePercent >= 0) {
        // הצגת אחוזים וסכומים
        const actualAmount = caseBalance;
        const expectedAmount = expectedAccumulation;
        arrearsValue.textContent = `${Math.round(compliancePercent)}% • ${formatter.format(actualAmount)} מתוך ${formatter.format(expectedAmount)}`;
        if (arrearsAmount > 0) {
          arrearsValue.textContent += ` (פיגור: ${formatter.format(arrearsAmount)})`;
        }
        arrearsValue.style.display = 'block';
        
        // עדכון הטקסט באחוזים בתוך הסרגל
        const percentText = Math.round(compliancePercent) + '%';
        arrearsText.textContent = percentText;
        
        // עדכון מלל סטטוס
        if (arrearsStatusText) {
          if (hasCredit || compliancePercent >= 100) {
            arrearsStatusText.textContent = 'יתרה תקינה';
            arrearsStatusText.style.color = '#10b981';
          } else if (arrearsPercent < 5) {
            arrearsStatusText.textContent = 'פיגור מינורי';
            arrearsStatusText.style.color = '#f59e0b';
          } else if (arrearsPercent < 30) {
            arrearsStatusText.textContent = 'פיגור בינוני';
            arrearsStatusText.style.color = '#f97316';
          } else {
            arrearsStatusText.textContent = 'פיגור משמעותי';
            arrearsStatusText.style.color = '#ef4444';
          }
          arrearsStatusText.style.fontWeight = '700';
        }
      } else {
        arrearsValue.textContent = 'אין נתונים';
        arrearsValue.style.display = 'block';
        arrearsGauge.style.width = '0%';
        arrearsGauge.style.backgroundColor = '#e2e8f0';
        if (arrearsText) arrearsText.textContent = '';
        if (arrearsStatusText) arrearsStatusText.textContent = '';
      }
    }
  }
}
...
function updateEffortMetric() {
  ...
  const scoreText = document.getElementById('effortScoreText');
  
  if (!statusText || !scoreValue || !scoreFill) return;
  
  // אם אין נתונים - הצג "אין נתונים"
  if (disposable <= 0 || recommendation <= 0) {
    statusText.textContent = '';
    scoreValue.textContent = 'אין נתונים';
    scoreValue.style.display = 'block';
    scoreFill.style.width = '0%';
    scoreFill.style.backgroundColor = '#e2e8f0';
  ...
  // הצגת אחוזים וסכומים
  if (scoreValue) {
    scoreValue.textContent = `${effortPercent.toFixed(1)}%`;
    scoreValue.style.display = 'block';
  }
  
  if (scoreText) {
    scoreText.textContent = `${effortPercent.toFixed(1)}%`;
  }
  ...
  // לוגיקת התראות חכמות עם סכומים
  if (effortPercent > 115) {
    statusText.innerHTML = `<span style='color:#dc2626; font-weight:800;'>⚠️ חשד להכנסות לא מדווחות (התשלום עולה על היכולת)</span><br><span style='font-size:11px; color:#64748b;'>המלצה: ${formatter.format(recommendation)} | הכנסה פנויה: ${formatter.format(disposable)}</span>`;
    statusText.style.color = '#dc2626';
    if (scoreFill) {
      scoreFill.style.backgroundColor = "#dc2626"; // אדום בוהק
    }
  } else if (effortPercent < 60) {
    statusText.innerHTML = `<span style='color:#eab308; font-weight:700;'>קיים פוטנציאל להגדלת התשלום</span><br><span style='font-size:11px; color:#64748b;'>המלצה: ${formatter.format(recommendation)} | הכנסה פנויה: ${formatter.format(disposable)}</span>`;
    statusText.style.color = '#eab308';
    if (scoreFill) {
      scoreFill.style.backgroundColor = "#eab308"; // צהוב
    }
  } else {
    statusText.innerHTML = `<span style='color:#15803d; font-weight:700;'>מאמץ תואם יכולת כלכלית</span><br><span style='font-size:11px; color:#64748b;'>המלצה: ${formatter.format(recommendation)} | הכנסה פנויה: ${formatter.format(disposable)}</span>`;
...
function calculateDynamicDividend() {
  ...
  const dividendDisplayValue = document.getElementById('dividendDisplayValue');
  ...
  // אם אין נתונים בסיסיים - הצג "אין נתונים"
  if (!hearingDateStr || (balance <= 0 && currentMonthly <= 0 && planTotal <= 0 && totalDebts <= 0)) {
    if (divText) divText.textContent = '';
    if (dividendDisplayValue) {
      dividendDisplayValue.textContent = 'אין נתונים';
      dividendDisplayValue.style.display = 'block';
    }
  ...
  // עדכון תוכנית נאמן - הצגת אחוזים וסכומים
  if (trusteePlanBar && trusteePlanText) {
    if (trusteeMonthly > 0 && trusteeDuration > 0 && generalDebts > 0) {
      // עדכון progress bar לפי אחוז הדיבידנד
      if (trusteeDivPercent > 0) {
        trusteePlanBar.style.width = Math.min(trusteeDivPercent, 100) + '%';
        trusteePlanText.textContent = `${trusteeDivPercent.toFixed(1)}%`;
      ...
      if (trusteeNetForGeneral <= 0 && priority > 0) {
        ...
        if (divText && (!commMonthly || !commDuration)) {
          divText.innerHTML = `<span style='font-weight:700;'>תוכנית נאמן: צפוי דיבידנד ${trusteeDivPercent.toFixed(1)}%</span><br><span style='font-size:11px; color:#64748b;'>קופה צפויה: ${formatter.format(trusteeExpectedFund)} | לנושים: ${formatter.format(trusteeNetForGeneral)}</span>`;
  ...
  // עדכון תוכנית ממונה - הצגת אחוזים וסכומים
  if (commPlanBar && commPlanText) {
    if (commMonthly > 0 && commDuration > 0 && generalDebts > 0) {
      // עדכון progress bar לפי אחוז הדיבידנד
      if (commDivPercent > 0) {
        commPlanBar.style.width = Math.min(commDivPercent, 100) + '%';
        commPlanText.textContent = `${commDivPercent.toFixed(1)}%`;
      ...
      if (commNetForGeneral <= 0 && priority > 0) {
        ...
        if (divText && (!trusteeMonthly || !trusteeDuration)) {
          divText.innerHTML = `<span style='font-weight:700;'>תוכנית ממונה: צפוי דיבידנד ${commDivPercent.toFixed(1)}%</span><br><span style='font-size:11px; color:#64748b;'>קופה צפויה: ${formatter.format(commExpectedFund)} | לנושים: ${formatter.format(commNetForGeneral)}</span>`;
          divText.style.color = "#1e293b";
        } else if (divText && trusteeMonthly > 0 && trusteeDuration > 0) {
          // אם יש שתי תוכניות - הצג השוואה עם סכומים
          const diff = trusteeDivPercent - commDivPercent;
          if (Math.abs(diff) > 0.1) {
            divText.innerHTML = `<span style='font-weight:700;'>השוואה: נאמן ${trusteeDivPercent.toFixed(1)}% | ממונה ${commDivPercent.toFixed(1)}% (הפרש: ${diff > 0 ? '+' : ''}${diff.toFixed(1)}%)</span><br><span style='font-size:11px; color:#64748b;'>נאמן: קופה ${formatter.format(trusteeExpectedFund)} | לנושים ${formatter.format(trusteeNetForGeneral)}</span><br><span style='font-size:11px; color:#64748b;'>ממונה: קופה ${formatter.format(commExpectedFund)} | לנושים ${formatter.format(commNetForGeneral)}</span>`;
            divText.style.color = "#1e293b";
          } else {
            divText.innerHTML = `<span style='font-weight:700;'>צפוי דיבידנד: ${trusteeDivPercent.toFixed(1)}%</span><br><span style='font-size:11px; color:#64748b;'>נאמן: קופה ${formatter.format(trusteeExpectedFund)} | לנושים ${formatter.format(trusteeNetForGeneral)}</span><br><span style='font-size:11px; color:#64748b;'>ממונה: קופה ${formatter.format(commExpectedFund)} | לנושים ${formatter.format(commNetForGeneral)}</span>`;
            divText.style.color = "#1e293b";
          }
        }
      }
  ...
  // עדכון ערך דיבידנד ראשי
  if (dividendDisplayValue) {
    const maxDivPercent = Math.max(trusteeDivPercent, commDivPercent);
    if (maxDivPercent > 0 && generalDebts > 0) {
      dividendDisplayValue.textContent = `${maxDivPercent.toFixed(1)}%`;
      dividendDisplayValue.style.display = 'block';
    } else if (!hearingDateStr || (balance <= 0 && currentMonthly <= 0 && planTotal <= 0 && totalDebts <= 0)) {
      dividendDisplayValue.textContent = 'אין נתונים';
      dividendDisplayValue.style.display = 'block';
    } else {
      dividendDisplayValue.textContent = 'אין נתונים';
