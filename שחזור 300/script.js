...
function updateTransparencyIndex() {
  ...
  repRadios.forEach(radio => {
    if (radio.checked) {
      ...
      if (label) {
        ...
        if (labelText.includes('כסדרם')) {
          reportsAsOrdered = true;
        } else if (labelText.includes('חלקי')) {
          reportsPartial = true;
        } else if (labelText.includes('לא הוגשו')) {
  ...
  if (transparencyGauge && transparencyText && transparencyValue) {
    ...
    // עדכון הטקסט - רק בתוך הסרגל
    const percentText = transparency + '%';
    transparencyText.textContent = percentText; // רק בתוך הסרגל
...
function updateArrearsGauge(compliancePercent, arrearsAmount, hasCredit = false) {
  ...
  if (arrearsGauge && arrearsText && arrearsValue) {
    ...
    let statusText = '';
    if (hasCredit) {
      arrearsGauge.classList.add('low'); // ירוק
      statusText = 'יתרת זכות';
    } else if (compliancePercent >= 95) {
      arrearsGauge.classList.add('low'); // ירוק
      statusText = 'עומד בתוכנית';
    } else if (compliancePercent >= 70) {
      arrearsGauge.classList.add('medium'); // כתום
...
// עדכון גרף צפי דיבידנד - עם לוגיקה חדשה (ניכוי דין קדימה + 15% הוצאות)
function updateDividendChart(data) {
  ...
  if (conductSection) {
    ...
    if (debtSubtitle) {
      ...
      if (debtRow) {
        ...
        // השדה הראשון הוא "סכום חוב", השני הוא "דין קדימה"
        if (debtInputs.length >= 1) {
          totalDebts = parseVal(debtInputs[0]?.value || '') || 0;
        }
        if (debtInputs.length >= 2) {
          priorityDebt = parseVal(debtInputs[1]?.value || '') || 0;
        }
      }
    }
  }
  ...
  if (totalDebts === 0 && priorityDebt === 0) {
    ...
    const conductCard = Array.from(document.querySelectorAll('.card')).find(card => {
  ...
  if (trusteePlanBar && trusteePlanText) {
    if (netDebts > 0) {
      ...
      if (isFinite(dividendPercent) && !isNaN(dividendPercent)) {
        ...
        trusteePlanText.style.display = dividendPercent > 5 ? 'block' : 'none';
      } else {
        trusteePlanBar.style.width = '0%';
        trusteePlanText.textContent = '0%';
  ...
  if (commPlanBar && commPlanText) {
    if (netDebts > 0) {
      ...
      if (isFinite(dividendPercent) && !isNaN(dividendPercent)) {
        ...
        commPlanText.style.display = dividendPercent > 5 ? 'block' : 'none';
      } else {
        commPlanBar.style.width = '0%';
        commPlanText.textContent = '0%';
...
function calculateAllMetrics() {
  ...
  let effortPercent = 0;
  if (disposableIncome > 0) {
    effortPercent = (totalPaymentToFund / disposableIncome) * 100;
  } else if (totalPaymentToFund > 0) {
  ...
  if (effortScoreStatusText) {
    ...
    if (disposableIncome > 0 && totalPaymentToFund > 0) {
      if (remainingDisposable < 1000) {
        ...
        statusColor = '#ef4444';
      } else if (remainingDisposable < 3000) {
        statusText = `משלם ${formatter.format(totalPaymentToFund)} מתוך הכנסה פנויה של ${formatter.format(disposableIncome)}`;
        statusColor = '#f59e0b';
      } else {
        statusText = `משלם ${formatter.format(totalPaymentToFund)} מתוך הכנסה פנויה של ${formatter.format(disposableIncome)}`;
        statusColor = '#64748b';
      }
    } else if (disposableIncome <= 0 && totalPaymentToFund > 0) {
      statusText = 'תשלום ללא הכנסה פנויה - נדרש בירור';
      statusColor = '#ef4444';
    } else {
      statusText = '';
    }
  ...
  if (ordinaryDebts > 0 && availableForOrdinary > 0) {
    ...
    if (!isFinite(dividendPercent) || isNaN(dividendPercent) || dividendPercent < 0) {
      dividendPercent = 0;
    }
  ...
  if (totalDebtsBar && totalDebtsText && expectedReturnBar && expectedReturnText && dividendPercentage) {
    ...
    if (ordinaryDebts > 0) {
      ...
      totalDebtsText.textContent = formatter.format(ordinaryDebts);
      totalDebtsText.style.display = totalDebtsPercent > 10 ? 'block' : 'none';
    } else {
      totalDebtsBar.style.width = '0%';
      totalDebtsText.textContent = '0';
      totalDebtsText.style.display = 'none';
    }
    ...
    if (availableForOrdinary > 0) {
      ...
      expectedReturnText.textContent = formatter.format(availableForOrdinary);
      expectedReturnText.style.display = expectedReturnPercent > 10 ? 'block' : 'none';
    } else {
      expectedReturnBar.style.width = '0%';
      expectedReturnText.textContent = '0';
      expectedReturnText.style.display = 'none';
    }
...
function restoreVersion() {
  ...
  if (confirm(`האם אתה בטוח שברצונך לשחזר את הגרסה "${versionName}"? כל השינויים הנוכחיים יאבדו.`)) {
    try {
      ...
      const folderName = version === '102' ? 'שחזור גרסה 102' : 'שחזרו 100';
      fetch(`./${folderName}/index.html`)
        ...
        .catch(error => {
          alert(`שגיאה בשחזור: ${error.message}\n\nאנא ודא שהתיקייה "${folderName}" קיימת בתיקיית הפרויקט.`);
        });
    } catch (error) {
      alert(`שגיאה בשחזור: ${error.message}\n\nאנא ודא שהתיקייה "${versionName}" קיימת בתיקיית הפרויקט.`);
    }
  }
}
